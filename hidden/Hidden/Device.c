...
NTSTATUS IrpDeviceControlHandler(PDEVICE_OBJECT  DeviceObject, PIRP  Irp)
{
	...
	// Other
	default:
		DbgPrint("FsFilter1!" __FUNCTION__ ": IOCTL hatasý :%08x\n", ioctl);
		status = STATUS_INVALID_PARAMETER;
		goto EndProc;
	}
...
EndProc:
	...
	if (NT_SUCCESS(status) && outputDataSize > 0)
	{
		...
		{
			DbgPrint("FsFilter1!" __FUNCTION__ ": bir þey oldu!\n");
			outputDataSize = outputDataMaxSize;
...
NTSTATUS InitializeDevice(PDRIVER_OBJECT DriverObject)
{
	...
	if (!NT_SUCCESS(status))
	{
		DbgPrint("FsFilter1!" __FUNCTION__ ": servis baþlatýlýrken hata:%08x\n", status);
		return status;
	}
